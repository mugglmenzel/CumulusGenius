...
import de.eorg.cumulusgenius.shared.cloudmapping.model.ahp.configuration.*;
...
public class UseCaseExperimentMultiComponentGlobalParallel {
        ...
        for (int i = STEP_SIZE; i <= NUM_APPLIANCES; i = i + STEP_SIZE)
            for (int j = STEP_SIZE; j <= NUM_SERVICES; j = j + STEP_SIZE) {
                ...
                if (components.size() > 1) {
                    ...
                    try {
                        // current work:
                        FormationValue best = computeFormations(
                                componentSolutions, numComps, i, j).last();
                        System.out
